...
public class BoardsService {
	...
	public Loves 좋아요(Loves loves) {
		lovesDao.insert(loves);// id, usersId, boardsId 완벽하게 동기화 할려면 SELECT를 해서 해야한다,
		return loves;
	}
	...
	public Boards 게시글수정화면데이터가져오기(Integer id) {
		...
		if(boards == null) {
		 ...
		 }//데이터를 응답하게되서 이상하게됨->Exception 종류를 나눔
	...
	public void 게시글수정하기(Integer id, UpdateDto updateDto) {
		...
		if (boardsPS == null) {
			throw new RuntimeException(id + "의 게시글을 찾을수없습니다.");
		} // -> 복기하는거 최초호출한 부분으로 들어가짐 -> return도 필요가 없다 catch 는 디스패텨 서블릿이 발동됨
